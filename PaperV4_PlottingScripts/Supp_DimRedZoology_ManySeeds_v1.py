"""
###############################################################################
    Supplemental Figure: A Zoology of Distortive Effects Across Seeds (v1)
###############################################################################

    Author: Eric Johnson
    Date Created: Thursday, April 1, 2021
...
###############################################################################
    ...
    In the supplemental version, I'm going to show this figure but across
    several random seeds so that it becomes a 4 x n_seeds figure.
...
if __name__ == "__main__":
    ...
    name_base = "V4SuppFig_DimRedZoology_ManySeeds_v1"
            ...
            for sNo in range(n_seeds):
                ...
                if color_by_cluster:
                    ...
                    for cNo, lab in enumerate(clust_2_label):
                        ...
                        med_Y = np.median(Y[sNo][good_idx], axis=0)
                        ...
                        ax_height  = back_axis.get_window_extent().height
                        ax_height /= fig.dpi
                        ...
                        bb = ax.text(x_loc, -0.005 - (cNo % 5) * text_h,
                                     cLab,
                                     # r"$N=$" + f"{sum(good_idx)}",
                                     ha='center', va='top',
                                     fontsize=10,
                    ...
                    if alg.lower() == 'tsne':
                        ...
                        [var_arr,
                         _, _] = pUtl.calc_N_nonuniform(sorted_PWD,
                                                        perp_arr=[param])
                    ...
                    if sNo == (n_seeds - 1):
                        cax = fig.colorbar(hax, ax=ax, cmap='magma',
                                           drawedges=False)

                        if alg.lower() == 'tsne':
                            cax.set_label(r"Effective Nearest Neighbors, " +
                                          r"$k_{Eff}$")
                        if alg.lower() == 'umap':
