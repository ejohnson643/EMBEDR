...
def make_figure_grid(fig_size=(7.2, 5.76), 
                      n_rows=2,
                      n_cols=2,
...
def add_median_labels(axis, Y, labels, clusters_2_label):
    for lNo, label in enumerate(clusters_2_label):
        cluster_number = f"{lNo + 1}"
        good_idx = labels == label
        tmpY = Y[good_idx]
        cluster_median = np.median(tmpY, axis=0)
        axis.text(*cluster_median,
                  f"{cluster_number}",
                  fontsize=12,
                  fontweight='bold',
                  va='center', ha='center')
...
def add_category_boxes(figure,
    ...
    ax_height = back_axis.get_window_extent().height / figure.dpi

    n_rows = len(main_axes)
    n_cols = len(main_axes[0])
    
    tmp_axis   = main_axes[0][0]
    rect_width = tmp_axis.get_window_extent().width
    rect_width = rect_width / ax_width * n_cols / 2.
    
    for lNo, label in enumerate(clusters_2_label):
        cluster_number = f"{lNo + 1}"
        
        if "Slamf1-Negative" in label:
            lab = " ".join(label.split(" Multipotent "))
        label_str = f"{lNo + 1}: " + label.title()
            
        if lNo < n_boxes_in_column:
            x_loc = rect_width / 2
            rect_x = 0
        else:
        ...
        y_loc = text_vert_pad - (lNo % n_boxes_in_column) * text_h
        bb = tmp_axis.text(x_loc, y_loc, label_str,
                           ha='center', va='top', fontsize=fontsize,
                           transform=back_axis.transAxes)

        text_h = (bb.get_size() + 2 * pad) / 72. / ax_height
        rect_y = ((lNo % n_boxes_in_column) + 1) * text_h

        label_cmap, label_mapper = processed_label[4], processed_label[3]
        label_color = label_cmap[label_mapper[label]]
