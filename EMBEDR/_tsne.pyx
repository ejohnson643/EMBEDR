...
cpdef tuple estimate_positive_gradient_nn(
    ...
    bint calc_error=False,
):
    cdef:
        Py_ssize_t n_samples = gradient.shape[0]
        Py_ssize_t n_dims = gradient.shape[1]
        double * diff
        double d_ij, p_ij, q_ij, d_kl = 0, sum_P = 0

        Py_ssize_t ii, jj, kk, dd
    ...
    with nogil, parallel(num_threads=num_threads):
        ...
        for ii in prange(n_samples, schedule="guided"):
            # Iterate over all the neighbors `j` and sum up their contribution
            for kk in range(indptr[ii], indptr[ii + 1]):
                jj = indices[kk]
                p_ij = P_data[kk]
                ...
                for dd in range(n_dims):
                    diff[dd] = embedding[ii, dd] - embedding[jj, dd]
                    d_ij = d_ij + diff[dd] * diff[dd]
                ...
                for dd in range(n_dims):
                    gradient[ii, dd] = gradient[ii, dd] + q_ij * p_ij * diff[dd]
                ...
                if calc_error:
                    sum_P += p_ij
                    d_kl  += p_ij * log((p_ij / (q_ij + EPSILON)) + EPSILON)
